K=hasattr
J='ed448 is not supported by this version of OpenSSL.'
I=classmethod
D=bytes
import abc as C
from C.B import A,B
...
class G(metaclass=C.ABCMeta):
	...
	def from_public_bytes(cls,data:D):
		...
		if not Î.ed448_supported():raise A(J,B.UNSUPPORTED_PUBLIC_KEY_ALGORITHM)
		return Î.ed448_load_public_bytes(data)
	@C.abstractmethod
...
if K(F,'ed448'):G.register(F.ed448.Ed448PublicKey)
class H(metaclass=C.ABCMeta):
	@I
	def generate(cls):
		from C.D.B.A.B import Î
		if not Î.ed448_supported():raise A(J,B.UNSUPPORTED_PUBLIC_KEY_ALGORITHM)
		return Î.ed448_generate_key()
	@I
	def from_private_bytes(cls,data:D):
		from C.D.B.A.B import Î
		if not Î.ed448_supported():raise A(J,B.UNSUPPORTED_PUBLIC_KEY_ALGORITHM)
		return Î.ed448_load_private_bytes(data)
	@C.abstractmethod
...
if K(F,'x448'):H.register(F.ed448.Ed448PrivateKey)