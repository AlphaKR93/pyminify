...
class AsyncRequestInterface:
	async def request(self,method:bytes|str,url:URL|bytes|str,*,headers:HeaderTypes=None,content:bytes|typing.AsyncIterator[bytes]|None=None,extensions:Extensions|None=None):
		method=enforce_bytes(method,name='method');url=enforce_url(url,name='url');headers=enforce_headers(headers,name='headers');headers=include_request_headers(headers,url=url,content=content);request=Request(method=method,url=url,headers=headers,content=content,extensions=extensions);response=await self.handle_async_request(request)
		try:await response.aread()
		finally:await response.aclose()
		return response
	@contextlib.asynccontextmanager
	async def stream(self,method:bytes|str,url:URL|bytes|str,*,headers:HeaderTypes=None,content:bytes|typing.AsyncIterator[bytes]|None=None,extensions:Extensions|None=None):
		method=enforce_bytes(method,name='method');url=enforce_url(url,name='url');headers=enforce_headers(headers,name='headers');headers=include_request_headers(headers,url=url,content=content);request=Request(method=method,url=url,headers=headers,content=content,extensions=extensions);response=await self.handle_async_request(request)
		try:yield response
		finally:await response.aclose()
	async def handle_async_request(self,request:Request):raise NotImplementedError
