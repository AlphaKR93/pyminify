    ...
    def is_removable_expression(self, node):
        ...
        if isinstance(node, ast.Subscript):
            ...
            if hasattr(ast, 'Index') and isinstance(slice_node, ast.Index):
                 ...
                 return ((not slice_node.lower or self.is_removable_expression(slice_node.lower)) and
                         (not slice_node.upper or self.is_removable_expression(slice_node.upper)) and
                         (not slice_node.step or self.is_removable_expression(slice_node.step)))
        ...
        if isinstance(node, ast.Dict):
            return all((k is None or self.is_removable_expression(k)) and self.is_removable_expression(v) 
